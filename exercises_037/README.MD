...
This is a walkthrough for metasploitable 1 machine. 

[Download Metasploitable v1](https://download.vulnhub.com/metasploitable/Metasploitable.zip) and install it.

  - User: user
  - Password: user

Choose any of the vulnerabilities and try to gain access to the machine.
...
Now that we have an authorized key, we need to find out the private key paired with the authorized public key.

We can get the archive of compromised key pairs provided by the folks at Offensive Security on their Github – RSA 2048.
